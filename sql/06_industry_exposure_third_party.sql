/*
Purpose:
  Export semiannual/annual third-party industry exposure for funds.

Key tables:
  CMFundThirdPartyIndPortfolio
  AShareIndustriesCode

Note:
  PRT_ENDDATE is usually xxxx0630 or xxxx1231.
*/
...
SELECT
    ...
    i.PRT_ENDDATE,
    i.IND_CODE,
    c.INDUSTRIESNAME AS industry_name,
    c.LEVELNUM AS industry_level,
    c.MEMO AS industry_memo,
    i.PRT_INDUSVALUE,
    i.PCT_NAV,
    i.CURRENCY_CODE
FROM fund_scope s
JOIN CMFundThirdPartyIndPortfolio i
  ON i.S_INFO_WINDCODE = s.F_INFO_WINDCODE
LEFT JOIN AShareIndustriesCode c
  ON SUBSTR(c.INDUSTRIESCODE, 1, 10) = i.IND_CODE
CROSS JOIN params p
WHERE i.PRT_ENDDATE BETWEEN p.start_dt AND p.end_dt
ORDER BY s.F_INFO_WINDCODE, i.PRT_ENDDATE, i.PCT_NAV DESC;
